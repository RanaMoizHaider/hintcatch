...
class extends Component {
    ...
    public function mount(AiModel $aiModel)
    {
        $this->aiModel = $aiModel;
        $this->name = $aiModel->name;
        $this->description = $aiModel->description;
        $this->provider_id = $aiModel->provider_id;
        $this->image = $aiModel->image;
        $this->color = $aiModel->color;
        $this->icon = $aiModel->icon;
        $this->release_date = $aiModel->release_date?->format('Y-m-d');
        $this->features = $aiModel->features ?? [];
    }

    protected function rules()
    {
        return [
            'name' => 'required|string|max:255|unique:ai_models,name,' . $this->aiModel->id,
    ...
    public function save()
    {
        ...
        session()->flash('success', 'AI Model updated successfully.');
