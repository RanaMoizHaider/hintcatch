...
<th {{ $attributes->merge(['class' => 'px-6 py-3 text-left']) }}>
    @if($sortable && $field)
        <flux:button wire:click="sort('{{ $field }}')" variant="ghost" size="sm" class="group flex items-center space-x-1 text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider hover:text-zinc-700 dark:hover:text-zinc-200">
            <span>{{ $slot }}</span>
            @if($sortBy === $field)
